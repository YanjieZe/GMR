...
spec=importlib.util.spec_from_file_location('anom_rep_v3_eval', str(ANOM)); anom=importlib.util.module_from_spec(spec); sys.modules['anom_rep_v3_eval']=anom; spec.loader.exec_module(anom)
MAN=json.load(open(ROOT/'logs/profile_tuning_v3/configs/manifest.json'))
BASE_OUT=ROOT/'logs/lafan1_config_refresh_validation'
RES=ROOT/'logs/profile_tuning_v3/results'
...
for cfg_name,info in sorted(MAN.items()):
    robot=info['robot']; group=info['group']; level=info['level']
    outdir=RES/cfg_name.replace('.json','')
    if not (outdir/'output.pkl').exists(): continue
    base_out=BASE_OUT/robot/'baseline'/'output.pkl'
    if not base_out.exists(): continue
    base_eval, det = evaluate_motion(base_out, robot, None, info['source'])
    cand_eval, _ = evaluate_motion(outdir/'output.pkl', robot, det, ROOT/'logs/profile_tuning_v3/configs'/cfg_name)
...
out=ROOT/'logs/profile_tuning_v3/representative_summary.json'
out.write_text(json.dumps(summary,indent=2))
lines=['# Representative profile tuning v3 summary','','## Per config','| robot | group | level | improvement | baseline score | candidate score | baseline normal p95 | candidate normal p95 |','|---|---|---|---:|---:|---:|---:|---:|']
for r in rows:
...
(ROOT/'logs/profile_tuning_v3/representative_summary.md').write_text('\n'.join(lines)+'\n')
print('\n'.join(lines))
